    ...
    <div class="work-area">
            ...
            <div class="step-container" in:fade>
                ...
                <div class="ingredients-list">
                    {#each ingredients as ing}
                        ...
                        <button
                            class="ingredient-btn {ing.added
                                ? 'added'
                                : ''} {locked ? 'locked' : ''}"
                            on:click={() => {
                                if (!locked) addIngredient(ing.id);
                            }}
                ...
                <div class="bowl ingredients-stage">
                    {#if ingredients.some((i) => i.added)}
                        <div
                            class="bowl-contents"
                            style="height: {ingredients.filter((i) => i.added)
                                .length * 20}%"
                        ></div>
                    {/if}
                </div>
        ...
        {:else if currentStep === "mix"}
            <div class="step-container" in:fade>
                ...
                <div
                    class="bowl mix-stage"
                    on:click={mix}
                    role="button"
                    tabindex="0"
                    on:keydown={(e) => e.key === "Enter" && mix()}
                >
                    <div
                        class="bowl-contents mixed"
                        style="transform: rotate({mixProgress * 2}deg)"
                    ></div>
                    {#if isMixing}
                        <div class="whisk" in:fly={{ y: -20, duration: 300 }}>
                            🥣
                        </div>
                    {/if}
                </div>
        ...
        {:else if currentStep === "decorate" || currentStep === "done"}
            <div class="step-container" in:fade>
                <h2 class="step-title">
                    {currentStep === "done" ? "Yummy!" : "Decorate!"}
                </h2>
                ...
                {#if currentStep === "decorate"}
                    <button class="finish-btn" on:click={finish}
                        >Add Toppings!</button
                    >
                {/if}
                {#if currentStep === "done"}
